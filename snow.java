...
public class SnowWord extends Application{
  ...
  public void start(Stage stage) throws Exception{
    WritableImage image = new WritableImage(1200, 400);
    drawString(image, "Snow Text", 60, 200);
    final PixelReader reader = image.getPixelReader();

    Canvas show = new Canvas(1200, 400);
    ...
    new AnimationTimer(){
      ...
      public void handle(long arg0){
        ...
        for(int i = 0; i < 10; i++){
          snow.add(new Point(Math.random() * 1200, 0, Math.random() + 0.5));
        }
      }
    }.start();
